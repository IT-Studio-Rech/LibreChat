...
function setupDevYaml() {
  ...
  const baseStat = fs.statSync(LIBRECHAT_YAML_PATH);
  const devStat = fs.existsSync(LIBRECHAT_DEV_YAML_PATH) ? fs.statSync(LIBRECHAT_DEV_YAML_PATH) : null;
  const shouldRegenerate = !devStat || baseStat.mtimeMs > devStat.mtimeMs;
  ...
  if (shouldRegenerate) {
    ...
    ok(devStat ? 'Updated librechat.dev.yaml (base yaml is newer)' : 'Created librechat.dev.yaml');
  } else {
    ok('librechat.dev.yaml up-to-date — skipped');
  }
